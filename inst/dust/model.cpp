...
public:
  ...
  size_t size() {
    return shared->dim_A + shared->dim_E + shared->dim_F + shared->dim_N + shared->dim_P + shared->dim_R + shared->dim_S + shared->dim_U + 10;
  }
  std::vector<real_type> initial(size_t step) {
    std::vector<real_type> state(shared->dim_A + shared->dim_E + shared->dim_F + shared->dim_N + shared->dim_P + shared->dim_R + shared->dim_S + shared->dim_U + 10);
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    for (int i = 1; i <= shared->dim_n_Si; ++i) {
      internal.n_Si[i - 1] = ((i > 1 ? S[i - 1 - 1] : 0)) - ((i < shared->n_group ? S[i - 1] : 0));
    }
    for (int i = 1; i <= shared->dim_n_Sx; ++i) {
      internal.n_Sx[i - 1] = std::round(S[i - 1] * shared->omega[i - 1] * shared->dt);
    }
    ...
    for (int i = 1; i <= shared->dim_dem_S; ++i) {
      internal.dem_S[i - 1] = std::round(internal.n_Si[i - 1] * shared->r_age * shared->dt) - internal.n_Sx[i - 1];
    }
    ...
    for (int i = 1; i <= shared->dim_lambda_1; ++i) {
      for (int j = 1; j <= shared->dim_lambda_2; ++j) {
        internal.lambda[i - 1 + shared->dim_lambda_1 * (j - 1)] = beta_t * shared->m[shared->dim_m_1 * (j - 1) + i - 1] * (A[j - 1] * shared->theta_A + S[j - 1] + P[j - 1]) / (real_type) N[j - 1];
      }
    }
    for (int i = 1; i <= shared->dim_n_Nx; ++i) {
      internal.n_Nx[i - 1] = internal.n_Ux[i - 1] + internal.n_Ex[i - 1] + internal.n_Ax[i - 1] + internal.n_Sx[i - 1] + internal.n_Px[i - 1] + internal.n_Fx[i - 1] + internal.n_Rx[i - 1];
    }
    ...
    for (int i = 1; i <= shared->dim_dem_N; ++i) {
      internal.dem_N[i - 1] = internal.dem_U[i - 1] + internal.dem_E[i - 1] + internal.dem_A[i - 1] + internal.dem_S[i - 1] + internal.dem_P[i - 1] + internal.dem_F[i - 1] + internal.dem_R[i - 1];
    }
    ...
    for (int i = 1; i <= shared->dim_n_S; ++i) {
      internal.n_S[i - 1] = dust::random::binomial<real_type>(rng_state, S[i - 1] + internal.dem_S[i - 1], 1 - std::exp(- shared->r_S[i - 1] * shared->dt));
    }
    ...
    for (int i = 1; i <= shared->dim_n_SR; ++i) {
      internal.n_SR[i - 1] = internal.n_S[i - 1];
    }
    ...
    for (int i = 1; i <= shared->dim_pharyngitis_inc_by_group; ++i) {
      internal.pharyngitis_inc_by_group[i - 1] = ((fmodr<real_type>(step, shared->steps_per_week) == 0 ? internal.n_ES[i - 1] : internal.pharyngitis_inc_by_group[i - 1] + internal.n_ES[i - 1]));
    }
    for (int i = 1; i <= shared->dim_S; ++i) {
      state_next[shared->offset_variable_S + i - 1] = S[i - 1] + internal.dem_S[i - 1] + internal.n_ES[i - 1] - internal.n_SR[i - 1];
    }
...
dust::pars_type<model> dust_pars<model>(cpp11::list user) {
  ...
  shared->dim_dem_S = shared->n_group;
  ...
  shared->dim_n_S = shared->n_group;
  shared->dim_n_SR = shared->n_group;
  shared->dim_n_Si = shared->n_group;
  shared->dim_n_Sx = shared->n_group;
  ...
  shared->offset_variable_F = shared->dim_A + shared->dim_E + shared->dim_P + shared->dim_S + shared->dim_U + 10;
  shared->offset_variable_N = shared->dim_A + shared->dim_E + shared->dim_F + shared->dim_P + shared->dim_R + shared->dim_S + shared->dim_U + 10;
  shared->offset_variable_P = shared->dim_A + shared->dim_E + shared->dim_S + shared->dim_U + 10;
  shared->offset_variable_R = shared->dim_A + shared->dim_E + shared->dim_F + shared->dim_P + shared->dim_S + shared->dim_U + 10;
  shared->offset_variable_S = shared->dim_A + shared->dim_E + shared->dim_U + 10;
  ...
  for (int i = 1; i <= shared->dim_N; ++i) {
    shared->initial_N[i - 1] = shared->U0[i - 1] + shared->A0[i - 1] + shared->E0[i - 1] + shared->S0[i - 1] + shared->P0[i - 1] + shared->F0[i - 1] + shared->R0[i - 1];
  }
  for (int i = 1; i <= shared->dim_P; ++i) {
  ...
  for (int i = 1; i <= shared->dim_r_S; ++i) {
    shared->r_S[i - 1] = 1 / (real_type) shared->delta_S;
  }
...
cpp11::sexp dust_info<model>(const dust::pars_type<model>& pars) {
  ...
  cpp11::writable::strings nms({"time", "infections_inc", "pharyngitis_inc", "scarlet_fever_inc", "igas_inc", "births_inc", "net_leavers_inc", "beta_t", "pharyngitis_scarlet_fever_rate", "scarlet_fever_rate", "U", "A", "E", "S", "P", "F", "R", "N"});
  ...
  dim[14] = cpp11::writable::integers({shared->dim_P});
  dim[15] = cpp11::writable::integers({shared->dim_F});
  dim[16] = cpp11::writable::integers({shared->dim_R});
  dim[17] = cpp11::writable::integers({shared->dim_N});
  dim.names() = nms;
  ...
  index[15] = integer_sequence(shared->offset_variable_F + 1, shared->dim_F);
