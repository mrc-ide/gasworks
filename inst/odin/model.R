...
update(S[])  <- S[i]  + dem_S[i]  + n_ES[i] - n_SR[i]
...
n_Nx[] <- n_Ux[i] + n_Ex[i] + n_Ax[i] + n_Sx[i] + n_Px[i] +
  n_Fx[i] + n_Rx[i]

dem_N[] <- dem_U[i] + dem_E[i] + dem_A[i] + dem_S[i] + dem_P[i] + dem_F[i] +
  dem_R[i]
...
lambda[, ] <- beta_t * m[i, j] *  (A[j] * theta_A + S[j] + P[j]) / N[j]
foi[] <- sum(lambda[i, ])
...
n_Si[] <- (if (i > 1) S[i - 1] else 0) - (if (i < n_group) S[i] else 0)
...
dem_S[] <- round(n_Si[i] * r_age * dt) - n_Sx[i]
...
n_S[] <- rbinom(S[i] + dem_S[i], 1 - exp(-r_S[i] * dt))
...
initial(F[]) <- F0[i]
initial(R[]) <- R0[i]
initial(N[]) <- U0[i] + A0[i] + E0[i] + S0[i] + P0[i] + F0[i] + R0[i]
