...
scarlet_fever_inc_by_group[] <- (
  if (step %% steps_per_week == 0) n_EF[i]
  else scarlet_fever_inc_by_group[i] + n_EF[i])
...
initial(F[])  <- F0[i]
initial(R[])  <- R0[i]
initial(N[])  <- U0[i] + A0[i] + E0[i] + S10[i] + S20[i] + F0[i] + R0[i]
